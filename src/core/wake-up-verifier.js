...
class WakeUpVerifier {
  ...
  _loadHistory() {
    try {
      ...
      console.warn('[WakeUpVerifier] History load failed, starting fresh:', e.message);
    }
  ...
  _saveHistory() {
    try {
      ...
      console.warn('[WakeUpVerifier] History save failed:', e.message);
    }
  }
...
if (require.main === module) {
  ...
  console.log('--- History ---');
  console.log(JSON.stringify(verifier.getHistory({ limit: 3 }), null, 2));
}
