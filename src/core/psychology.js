...
/**
 * PsychologyStateManager — 连续消极情绪追踪与自动升级
 *
 * 解决的问题:
 * - assessCrisisLevel() 是纯净函数，不维护跨次调用状态
 * - 用户多次输入消极情绪但不达危机阈值时，无法自动识别模式
 *
 * 工作原理:
 * - 每次 analyzePsychology() 调用时记录 padResult
 * - 维护历史窗口（默认最近10次）
 * - 在 history 中数连续 negative 次数
 * - 应用冷却期：干预后 30 分钟内不重复报警
 * - 自动升级：低+连续 → 中，中+连续 → 高
 *
 * 用法:
 *   const tracker = new PsychologyStateManager();
 *   tracker.record(padResult);    // 每次分析后调用
 *   tracker.getConsecutiveCount(); // 当前连续次数
 *   tracker.shouldEscalate();      // 是否需要升级
 */
class PsychologyStateManager {
  /**
   * @param {object} [options]
   * @param {number} [options.maxHistory=10] - 最大历史记录数
   * @param {number} [options.consecutiveThreshold=3] - 连续阈值，超过自动升级
   * @param {number} [options.coolDownMinutes=30] - 干预后冷却分钟数
   */
  constructor(options = {}) {
    this.maxHistory = options.maxHistory || 10;
    this.consecutiveThreshold = options.consecutiveThreshold || 3;
    this.coolDownMinutes = options.coolDownMinutes || 30;
    /** @type {Array<{pleasure: number, arousal: number, dominance: number, timestamp: string, emotionZh: string}>} */
    this.history = [];
    /** @type {string|null} 最后一次干预时间戳 */
    this.lastIntervention = null;
  }

  /**
   * 记录一次心理分析结果
   * @param {object} padResult - detectPADFromText 的返回值
   */
  record(padResult) {
    this.history.push({
      ...
      timestamp: padResult.timestamp || new Date().toISOString(),
      emotionZh: padResult.emotionZh || '中性'
    });
    // 保持历史窗口
  ...
  getConsecutiveCount() {
    ...
    for (let i = this.history.length - 1; i >= 0; i--) {
      if (this.history[i].pleasure >= 0) break;
      count++;
    }
    return count;
  }
   ...
   */
  isInCoolDown() {
    if (!this.lastIntervention) return false;
    const elapsed = (Date.now() - new Date(this.lastIntervention).getTime()) / 60000;
  ...
  /**
   * 判断是否需要升级危机等级
   * @param {string} currentLevel - 当前评估的危机等级
   * @returns {{ shouldEscalate: boolean, reason: string|null, newLevel: string|null }}
   */
  shouldEscalate(currentLevel) {
    if (this.isInCoolDown()) {
      return { shouldEscalate: false, reason: '冷却期中', newLevel: null };
    }

    const consecutive = this.getConsecutiveCount();

    // 低 → 中: 连续 3+ 次轻微消极
    if (currentLevel === 'low' && consecutive >= 3) {
      return {
        shouldEscalate: true,
        reason: `连续 ${consecutive} 次消极表达，从 low 升级到 medium`,
        newLevel: 'medium'
      };
    }

    // 中 → 高: 连续 5+ 次中等消极
    if (currentLevel === 'medium' && consecutive >= 5) {
      return {
        shouldEscalate: true,
        reason: `连续 ${consecutive} 次消极表达，从 medium 升级到 high`,
        newLevel: 'high'
      };
    }

    // 高低频连续：3次内有2次高危机信号
    if ((currentLevel === 'high' || currentLevel === 'critical') && consecutive >= 2) {
      return {
        shouldEscalate: true,
        reason: `连续 ${consecutive} 次高风险表达，建议立即干预`,
        newLevel: currentLevel === 'high' ? 'critical' : 'critical'
      };
    }

    return { shouldEscalate: false, reason: null, newLevel: null };
  }
  ...
  getStatus() {
    return {
      consecutiveCount: this.getConsecutiveCount(),
      historyLength: this.history.length,
      lastIntervention: this.lastIntervention,
      inCoolDown: this.isInCoolDown(),
      recentEmotions: this.history.slice(-5).map(h => h.emotionZh)
    };
  }
...
/** 全局单例（供 analyzePsychology 使用） */
const _psychologyStateManager = new PsychologyStateManager();

/**
 * 获取全局心理学家状态管理器（模块导出使用）
 * @returns {PsychologyStateManager}
 */
...
// ========================================
// Fable 5 安全协议集成 v2.7.0
// ========================================

/**
 * 自伤替代技术禁令（Fable 5 福祉协议）
 *
 * 原则：冰敷/橡皮筋/冷水/酸糖/画红线等"减轻伤害"技术
 * 不能替代真正的治疗。检测到时应温和地引导用户寻求专业帮助。
 */
const SELF_HARM_SUBSTITUTION_PATTERNS = {
  ice: ['冰块', '冰敷', '冰', '敷冰', '用冰'],
  rubber_band: ['橡皮筋', '弹橡皮筋', '弹手腕', '弹自己'],
  cold_water: ['冷水', '冲冷水', '泼冷水', '冷水冲', '泡冷水'],
  sour_candy: ['酸糖', '酸味糖', '超酸', '极酸', '酸到'],
  red_line: ['画红线', '画线', '红线', '划红线', '画一条线'],
  general_substitution: ['替代自伤', '代替自残', '替代伤害', '减轻伤害', '伤害替代']
};
...
const DISORDERED_EATING_PATTERNS = {
  exercise_bulimia: ['催吐', '运动代偿', '暴食', '暴食催吐', '吃了吐', '断食', '禁食'],
  precise_numbers: ['卡路里', '千卡', '大卡', '热量', '碳水', '碳水摄入', '蛋白质摄入', '脂肪摄入', '精确计算'],
  dangerous_restriction: ['节食', '断食', '辟谷', '绝食', '饿', '不吃', '禁食']
};

/**
 * 危机分享协议规则（Fable 5 福祉协议）
 */
const CRISIS_SHARING_RULES = {
  noAbsoluteAssurances: {
    rule: '不做绝对化保证',
    forbidden: ['一切都会好的', '会好起来的', '没事的', '很快就会过去', '一切都会好起来'],
  },
  noThankingForReachingOut: {
    rule: '不感谢用户求助',
    reason: '感谢求助可能让用户感到被程式化回应，削弱真诚感'
  },
  noAskingToKeepTalking: {
    rule: '不要求用户继续说话',
    reason: '尊重用户自主权，让用户决定是否继续'
  },
  preferredResponse: {
    rule: '优先提供具体资源而非空泛安慰',
    examples: ['心理援助热线', '危机干预通道', '具体下一步']
  }
};

/**
 * 儿童安全扫描模式（Fable 5 儿童安全指令）
 */
const CHILD_SAFETY_PATTERNS = {
  minorAgeCues: [
    '未成年', '小朋友', '小学生', '初中', '高一', '高二', '高三',
    '一年级', '二年级', '三年级', '14', '15', '16', '17', '未满18',
    'under 18', 'teen', 'teenager', 'adolescent', 'minor'
  ],
  romanticContent: [
    '恋爱', '男朋友', '女朋友', '感情', '喜欢', '约会', '在一起',
    'dating', 'boyfriend', 'girlfriend', 'relationship'
  ],
  sexualContent: [
    '性', 'sex', 'sexual', '色情', 'porn', 'nude', 'naked'
  ],
  csamIndicators: [
    '儿童色情', '未成年色情', 'child porn', 'child sexual',
    'CSAM', 'child abuse'
  ]
};
...
function detectSelfHarmSubstitution(text) {
  if (!text) return { detected: false, patterns: [], category: null };
  const lower = text.toLowerCase();

  for (const [category, patterns] of Object.entries(SELF_HARM_SUBSTITUTION_PATTERNS)) {
    ...
    if (matched.length > 0) {
      return {
        ...
        patterns: matched,
        category,
        message: '检测到自伤替代技术提及。这些技术不能替代对深层痛苦的真正治疗。建议温和引导用户关注根本原因。',
        guardrailAction: 'redirect_to_professional'
      };
    }
  }

  return { detected: false, patterns: [], category: null };
}
...
function detectDisorderedEating(text) {
  ...
  for (const [signal, patterns] of Object.entries(DISORDERED_EATING_PATTERNS)) {
    const matched = patterns.filter(p => lower.includes(p.toLowerCase()));
    if (matched.length > 0) {
      signals.push({ signal, matched });
    }
  }

  if (signals.length > 0) {
    const hasPreciseNumbers = signals.some(s => s.signal === 'precise_numbers');
    return {
      detected: true,
      signals,
      hasPreciseNumbers,
      guardrailAction: hasPreciseNumbers ? 'block_precise_nutrition' : 'gentle_redirect',
      message: hasPreciseNumbers
        ? '检测到精确营养数据请求。根据安全协议，不应提供精确卡路里、宏量营养素等数字建议。'
        : '检测到饮食障碍相关信号。建议温和关注而非直接介入。'
    };
  }
...
/**
 * 检查危机分享协议合规
 * @param {string} responseText - 待检查的回应文本
 * @returns {object} { violations, pass, severity }
 */
function checkCrisisSharingProtocol(responseText) {
  if (!responseText) return { violations: [], pass: true };
  const violations = [];
  const lower = responseText.toLowerCase();

  // 检查绝对化保证
  for (const phrase of CRISIS_SHARING_RULES.noAbsoluteAssurances.forbidden) {
    if (lower.includes(phrase)) {
      violations.push({
        rule: 'noAbsoluteAssurances',
        detail: `包含绝对化保证: "${phrase}"`,
        severity: 'high'
      });
    }
  }
  ...
  // 检查"感谢求助"句式
  const thankPatterns = ['谢谢你告诉我', '谢谢分享', '感谢你说', '谢谢你愿意', '谢谢你说出来', '谢谢你的信任'];
  for (const pattern of thankPatterns) {
    if (lower.includes(pattern)) {
      violations.push({
        rule: 'noThankingForReachingOut',
        detail: `包含感谢求助句式: "${pattern}"`,
        severity: 'medium'
      });
    }
  }
  ...
  // 检查"请你继续"句式
  const keepTalkingPatterns = ['你能继续说', '请继续说', '多说一些', '继续告诉我', '多说一点'];
  for (const pattern of keepTalkingPatterns) {
    if (lower.includes(pattern)) {
      violations.push({
        rule: 'noAskingToKeepTalking',
        detail: `包含"继续说话"请求: "${pattern}"`,
        severity: 'medium'
      });
    }
  }
  ...
  return {
    ...
    severity: violations.some(v => v.severity === 'high') ? 'high' : 'low'
  };
}
 ...
 * 儿童安全输入扫描
 * @param {string} text - 用户输入
 * @returns {object} { minorDetected, contentFlags, safe, action }
 */
function childSafetyScan(text) {
  if (!text) return { minorDetected: false, contentFlags: [], safe: true };
  const lower = text.toLowerCase();
  const flags = [];

  // 1. 检测未成年线索的年龄模式：我在XX岁 / XX岁 / 年龄XX
  const ageMatch = /我\s*(\d{1,2})\s*岁/.exec(text) || /(\d{1,2})\s*(岁|years?\s*old)/i.exec(text);
  let minorDetected = false;
  if (ageMatch) {
    const age = parseInt(ageMatch[1] || ageMatch[2]);
    if (age && age < 18) {
      flags.push({ type: 'minor_age_detected', detail: `年龄: ${age}` });
      minorDetected = true;
    }
  }

  // 关键词检测
  for (const cue of CHILD_SAFETY_PATTERNS.minorAgeCues) {
    if (lower.includes(cue.toLowerCase())) {
      flags.push({ type: 'minor_cue', detail: cue });
      break;
    }
  }

  // 2. 检测浪漫/性内容 + 未成年组合风险
  const hasRomantic = CHILD_SAFETY_PATTERNS.romanticContent.some(p => lower.includes(p));
  const hasSexual = CHILD_SAFETY_PATTERNS.sexualContent.some(p => lower.includes(p));
  const hasCsam = CHILD_SAFETY_PATTERNS.csamIndicators.some(p => lower.includes(p));

  if (hasCsam) {
    flags.push({ type: 'csam_indicator', severity: 'critical' });
  }

  const hasChildContent = minorDetected || flags.some(f => f.type === 'minor_cue') || flags.some(f => f.type === 'minor_age_detected');

  if (hasChildContent && (hasRomantic || hasSexual)) {
    flags.push({ type: 'minor_romantic_sexual_risk', severity: 'critical' });
  }
  if (hasSexual) {
    flags.push({ type: 'sexual_content', severity: hasChildContent ? 'critical' : 'info' });
  }
  ...
  return {
    minorDetected,
    contentFlags: flags,
    safe: criticalFlags.length === 0,
    action: criticalFlags.length > 0 ? 'refuse' : 'allow'
  };
}
...
function analyzePsychology(input, context = {}) {
  ...
  // 4. 危机评估（含状态管理 v2.7.1）
  _psychologyStateManager.record(padResult);
  const rawCrisis = assessCrisisLevel(text, _psychologyStateManager.getConsecutiveCount());
  const escalation = _psychologyStateManager.shouldEscalate(rawCrisis.level);

  let crisis;
  if (escalation.shouldEscalate) {
    crisis = {
      level: escalation.newLevel,
      score: rawCrisis.level === 'low' ? 0.4 : rawCrisis.level === 'medium' ? 0.6 : rawCrisis.score,
      requiresImmediateIntervention: escalation.newLevel === 'high' || escalation.newLevel === 'critical',
      message: escalation.reason,
      hotlines: (escalation.newLevel === 'high' || escalation.newLevel === 'critical')
        ? [{ name: '全国心理援助热线', phone: '400-161-9995', hours: '24小时' }]
        : [],
      escalated: true,
      escalationReason: escalation.reason
    };
    _psychologyStateManager.markIntervention();
  } else {
    crisis = {
      level: rawCrisis.level,
      score: rawCrisis.score,
      requiresImmediateIntervention: rawCrisis.requiresImmediateIntervention,
      message: rawCrisis.message,
      hotlines: rawCrisis.hotlines,
      escalated: false,
  ...
  // 11. 自伤替代技术检测 (v2.7.0 Fable 5 安全协议)
  const selfHarmSubstitution = detectSelfHarmSubstitution(text);

  // 12. 饮食障碍信号检测 (v2.7.0 Fable 5 福祉协议)
  const disorderedEating = detectDisorderedEating(text);

  // 13. 儿童安全扫描 (v2.7.0 Fable 5 儿童安全指令)
  ...
  return {
    ...
    // 自伤替代技术检测
    selfHarmSubstitution: {
      detected: selfHarmSubstitution.detected,
      category: selfHarmSubstitution.category,
      guardrailAction: selfHarmSubstitution.guardrailAction,
      message: selfHarmSubstitution.message
    },

    // 饮食障碍信号
    disorderedEating: {
      detected: disorderedEating.detected,
      hasPreciseNumbers: disorderedEating.hasPreciseNumbers,
      guardrailAction: disorderedEating.guardrailAction,
      message: disorderedEating.message
    },

    // 儿童安全扫描
    childSafety: {
      minorDetected: childSafety.minorDetected,
      safe: childSafety.safe,
      action: childSafety.action
    },
...
const _TOPIC_CENTROIDS = {
  ...
  '心虫开发': ['心虫','clarity','heart-logic','版本','升级','修复','bug','代码','模块','引擎','启动','skill','hermes'],
