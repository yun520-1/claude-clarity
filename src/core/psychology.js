...
function analyzePsychology(input, context = {}) {
  ...
  // 4. 危机评估
  if (padResult && padResult.pleasure < -2) _consecutiveNegativeCount++;
  else if (padResult && padResult.pleasure >= -2 && _consecutiveNegativeCount > 0) _consecutiveNegativeCount--;
  const rawCrisis = assessCrisisLevel(text, _consecutiveNegativeCount);
  const crisis = {
