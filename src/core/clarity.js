...
const { ALLOWED_ROUTES, LAZY_TIER2, EAGER_NAMES, LAZY_NAMES } = require('./clarity-routes.js');
...
const _ClarityDecision = _lazy('clarityDecision', () => require('./decision.js'));
...
class Clarity {
  ...
  _initLazyRegistry() {
    ...
    add('decision',         () => new (_ClarityDecision().ClarityDecision)(_self.memory));
...
add('deliberationGate', () => new (_DeliberationGate().DeliberationGate)());
  ...
  start() {
    ...
    if (identityResult.success) {
      ...
      console.warn(`[Clarity] 身份核心加载部分失败:`, identityResult.errors);
    }
  ...
  _bootMindSpace() {
    ...
    if (this._mindSpace.rules.length === 0) {
      ...
      if (retryRules.length === 0) {
        console.warn('[Clarity] 无法初始化 MindSpace 身份规则（memory 可能未就绪）');
      } else {
  ...
  async think(input, depth) {
    ...
    const startPatterns = /^(启动心虫|开机|activate|start clarity|开启心虫)/i;
  ...
  /**
   * 查询对话历史（按时间范围）
   * @param {object} opts - { since: timestamp, until: timestamp, role: 'user'|'clarity', limit: 50 }
   */
...
function createClarity(config = {}) {
  return new Clarity(config);
}

// CLI
if (require.main === module) {
  const rootPath = path.join(__dirname, '..', '..');
  const hf = createClarity({ rootPath });
  hf.start();
...
module.exports = { Clarity, createClarity, VERSION: _VERSION().VERSION, MentalEffortTracker: _MentalEffortTracker().MentalEffortTracker };
