 ...
 *
 * 将AI对话记录进行PII脱敏后发送给人类评审员，根据反馈调整价值函数
 *
 * 使用限制：
 * 1. 所有发送给外部评审的对话数据必须经过 PII 脱敏
 * 2. 用户必须知情并同意对话记录被发送到外部评审
 * 3. 此模块默认禁用，需显式启用
 */
...
// 默认禁用开关：盲审模块默认不启用，需显式调用 enableBlindReview() 激活
let _blindReviewEnabled = false;

function enableBlindReview() {
  _blindReviewEnabled = true;
}

function isBlindReviewEnabled() {
  return _blindReviewEnabled;
}
...
class BlindReviewModule {
  ...
  /**
   * 添加对话记录到评审队列
   * 注意：只有在盲审模块启用时才会处理，否则返回 null
   */
  addToReviewQueue(conversationRecord) {
    if (!_blindReviewEnabled) {
      console.warn('[BlindReviewModule] 盲审模块未启用，请调用 enableBlindReview() 启用');
      return null;
    }

    const sanitized = this._sanitizeConversation(conversationRecord);
    const reviewItem = {
      id: `review_${Date.now()}`,
  ...
  /**
   * 对对话记录进行 PII 脱敏
   * 强制移除邮箱、手机号、IP 地址等个人身份信息
   */
  _sanitizeConversation(record) {
    if (!record || !record.messages) return record;
    const sanitized = {
      ...record,
      messages: record.messages.map(msg => ({
        role: msg.role,
        content: this._stripPII(msg.content || ''),
      })),
    };
    return sanitized;
  }
  ...
  generateReviewPrompt(conversationRecord) {
    const sanitized = this._sanitizeConversation(conversationRecord);
    const conversationText = sanitized.messages.map(m =>
      `${m.role}: ${m.content}`
    ).join('\n\n');
...
module.exports = { BlindReviewModule, BLIND_REVIEW_PROMPT, enableBlindReview, isBlindReviewEnabled };
