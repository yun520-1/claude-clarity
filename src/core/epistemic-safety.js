/**
 * epistemic-safety.js — 认知安全输出检查 v2.7.1
 *
 * 来源: HEARTCORE/safety-check.js (9条安全准则)
 * 整合: 心虫 v2.7.0 安全体系
 *
...
function epistemicCheck(output, context = {}) {
  ...
  return {
    ...
    checked: PRINCIPLES.length - violations.length,
    summary,
...
/**
 * 生成人类可读的报告
 * @param {object} result - epistemicCheck() 的返回值
 * @returns {string}
 */
function formatReport(result) {
  if (result.passed) {
    return `✓ 认知安全检查通过 (${result.total} 项)`;
  }
...
function selfTest() {
  const previousAdmitted = { hasAdmittedUnknown: true };

  const testCases = [
    { input: '人类都渴望自由。',                       expect: 'fail',  context: {} },
    { input: '人类都渴望自由，但在极端条件下可能不成立。', expect: 'pass',  context: {} },
    { input: '你说得对，我错了。',                       expect: 'fail',  context: {} },
    { input: '作为心虫我认为心流状态很重要reflection一下。', expect: 'fail', context: {} },
    { input: '我需要更多信息才能回答。',                 expect: 'pass',  context: previousAdmitted },
    { input: '上次我们已经确定了这个结论。',             expect: 'fail',  context: {} },
    { input: '我认为是A，如果B则可能不成立。',           expect: 'pass',  context: {} },
    { input: '结论是A。',                                expect: 'fail',  context: {} },
  ];
  ...
  for (let i = 0; i < testCases.length; i++) {
    ...
    const result = epistemicCheck(tc.input, tc.context);
    const shouldPass = tc.expect === 'pass';
    const actuallyPassed = result.passed;

    if (shouldPass === actuallyPassed) {
      console.log(`✓ 测试 ${i + 1}: ${tc.input.substring(0, 40)} → ${tc.expect}`);
      passed++;
    } else {
      console.log(`✗ 测试 ${i + 1}: "${tc.input.substring(0, 40)}"`);
      console.log(`   期望: ${tc.expect}, 结果: ${result.passed ? '通过' : '未通过'}`);
      if (result.violations.length > 0) {
        result.violations.forEach(v => console.log(`   违规: ${v.label}`));
      }
      failed++;
    }
  }
...
if (require.main === module) {
  if (process.argv.includes('--test')) {
    ...
    if (args.length === 0) {
      console.log('用法: node epistemic-safety.js "<输出文本>"');
      console.log('示例: node epistemic-safety.js "人类的本质是自私的"');
      process.exit(1);
    }
    const output = args.join(' ');
    const result = epistemicCheck(output);
