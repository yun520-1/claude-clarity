...
```bash
*/23 * * * * cd ~/.jvs/.openclaw/workspace/claude-heartflow-skill && ./cron/auto-evolution.sh >> ~/heartflow-cron.log 2>&1
```
...
```bash
0 9 * * * cd ~/.jvs/.openclaw/workspace/claude-heartflow-skill && node scripts/personality-check.js status >> ~/heartflow-daily.log 2>&1
```
...
```bash
*/30 * * * * cd ~/.jvs/.openclaw/workspace/claude-heartflow-skill && node scripts/sync-personality-from-main.js >> ~/heartflow-sync.log 2>&1
```
...
- **详细日志**: `claude-heartflow-skill/logs/`
